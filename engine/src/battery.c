...
static int aes_try_master(const uint8_t *master, uint32_t klen,
    ...
    uint32_t mode;
    if (engine_try_block_modes(&bc, pt, ct, len, &mode))
        return fill(v, master, klen, alg, mode, 0);
    return 0;
}
...
static int block_try(block_cipher_ctx_t *bc, const uint8_t *key, uint32_t klen,
    ...
    uint32_t mode;
    if (engine_try_block_modes(bc, pt, ct, len, &mode))
        return fill(v, key, klen, alg, mode, 0);
    return 0;
}
...
static int try_block_pair(const uint8_t *a, const uint8_t *b, const uint8_t *pt,
    ...
    if (aes_window_battery(k32, 32, SAR_ALG_AES_256, 7, pt, ct, len, v))
        return 1;
    if (aes_window_battery(k24a, 24, SAR_ALG_AES_192, 7, pt, ct, len, v))
        return 1;
    if (aes_window_battery(k24b, 24, SAR_ALG_AES_192, 7, pt, ct, len, v))
        return 1;

    {
        ...
        if (block_try(&bc, k24a, 24, SAR_ALG_3DES, pt, ct, len, v)) return 1;
        des3_setkey(&d, k24b);
        if (block_try(&bc, k24b, 24, SAR_ALG_3DES, pt, ct, len, v)) return 1;
    }
    {
        camellia_ctx_t c; camellia_key_schedule(&c, k32, 32);
        block_cipher_ctx_t bc = { camellia_encrypt_block, camellia_decrypt_block, &c, 16 };
        if (block_try(&bc, k32, 32, SAR_ALG_CAMELLIA, pt, ct, len, v)) return 1;
    }
    {
        aria_ctx_t ar; aria_key_schedule(&ar, k32, 32);
        block_cipher_ctx_t bc = { aria_encrypt_block, aria_decrypt_block, &ar, 16 };
        if (block_try(&bc, k32, 32, SAR_ALG_ARIA, pt, ct, len, v)) return 1;
    }
...
static int try_chacha(const uint8_t *gen_key, const uint8_t *report_key,
    ...
    for (int ri = 0; ri < 3; ri++) {
        ...
        if (match)
            return fill(v, report_key, 32, alg, SAR_MODE_STREAM, 0);
    }
    return 0;
}

static int try_salsa(const uint8_t *gen_key, const uint8_t *report_key,
                     const uint8_t *nonce8, const uint8_t *pt,
                     const uint8_t *ct, uint32_t sample_size, uint64_t file_offset,
                     uint32_t alg, sar_verdict_t *v) {
    uint64_t ctr = file_offset / 64;
    uint32_t boff = (uint32_t)(file_offset % 64);
    uint8_t ks[64];
    salsa_block(gen_key, nonce8, ctr, 20, ks);
    uint32_t clen = 64 - boff;
    if (clen > sample_size) clen = sample_size;
    if (clen < 16) return 0;
    for (uint32_t m = 0; m < clen; m++)
        if ((pt[m] ^ ks[boff + m]) != ct[m]) return 0;
    return fill(v, report_key, 32, alg, SAR_MODE_STREAM, 0);
}
...
static int try_stream(const uint8_t (*cands)[16], size_t count, const uint8_t *pt,
    ...
    for (size_t i = 0; i + 1 < count; i++) {
        ...
        for (size_t k = 0; k < count; k++) {
            ...
            if (try_chacha(key32, key32, nonce12, pt, ct, sample_size, file_offset,
                           SAR_ALG_CHACHA20, v)) return 1;
            uint8_t nonce8[8];
            sar_memcpy(nonce8, cands[k], 8);
            if (try_salsa(key32, key32, nonce8, pt, ct, sample_size, file_offset,
                          SAR_ALG_SALSA20, v)) return 1;
            if (k + 1 < count) {
                uint8_t nonce24[24];
                ...
                if (try_chacha(subkey, key32, derived12, pt, ct, sample_size,
                               file_offset, SAR_ALG_XCHACHA20, v)) return 1;
                hsalsa20(key32, nonce24, subkey);
                sar_memcpy(derived8, nonce24 + 16, 8);
                if (try_salsa(subkey, key32, derived8, pt, ct, sample_size,
...
static int scan_battery(const uint8_t *buf, size_t buf_len, const uint8_t *pt,
    ...
    for (size_t off = 0; off + 16 <= buf_len; off += 16) {
        ...
        if (try_block_single(buf + off, pt, ct, sample_size, v))
            return 1;
...
int sar_convict(const sar_engine_input_t *in, sar_verdict_t *out) {
    ...
    for (size_t i = 0; i < count; i++) {
        if (!candidate_viable(cands[i])) continue;
        if (try_block_single(cands[i], in->plaintext, in->ciphertext, len, out))
            return 1;
    }
