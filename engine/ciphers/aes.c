#include "aes.h"
#include "../src/eng_mem.h"

static const uint8_t SBOX[256] = {
    
};

static const uint8_t INV_SBOX[256] = {
    
};

static const uint8_t RCON[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ ((x >> 7) * 0x1b));
}

static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi = (uint8_t)(a & 0x80);
        a = (uint8_t)(a << 1);
        if (hi) a ^= 0x1b;
        b = (uint8_t)(b >> 1);
    }
    return p;
}

void aes_setkey(aes_ctx_t *ctx, const uint8_t *key, uint32_t key_len) {
    uint32_t nk = key_len / 4;
    int nr = (int)nk + 6;
    ctx->nr = nr;
    uint8_t *w = ctx->rk;
    sar_memcpy(w, key, key_len);
    uint32_t total = 4u * (uint32_t)(nr + 1);
    for (uint32_t i = nk; i < total; i++) {
        uint8_t t[4];
        sar_memcpy(t, w + (i - 1) * 4, 4);
        if (i % nk == 0) {
            uint8_t r = t[0];
            t[0] = (uint8_t)(SBOX[t[1]] ^ RCON[i / nk]);
            t[1] = SBOX[t[2]];
            t[2] = SBOX[t[3]];
            t[3] = SBOX[r];
        } else if (nk > 6 && i % nk == 4) {
            t[0] = SBOX[t[0]]; t[1] = SBOX[t[1]];
            t[2] = SBOX[t[2]]; t[3] = SBOX[t[3]];
        }
        for (int j = 0; j < 4; j++)
            w[i * 4 + j] = (uint8_t)(w[(i - nk) * 4 + j] ^ t[j]);
    }
}

static void add_round_key(uint8_t *s, const uint8_t *rk) {
    for (int i = 0; i < 16; i++) s[i] ^= rk[i];
}

int aes_encrypt_block(void *vctx, const uint8_t *in, uint8_t *out) {
    aes_ctx_t *ctx = (aes_ctx_t *)vctx;
    uint8_t s[16];
    sar_memcpy(s, in, 16);
    add_round_key(s, ctx->rk);
    for (int round = 1; round <= ctx->nr; round++) {
        for (int i = 0; i < 16; i++) s[i] = SBOX[s[i]];
        uint8_t t;
        t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
        t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
        t = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = s[3]; s[3] = t;
        if (round != ctx->nr) {
            for (int c = 0; c < 4; c++) {
                uint8_t *col = s + c * 4;
                uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
                col[0] = (uint8_t)(xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3);
                col[1] = (uint8_t)(a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3);
                col[2] = (uint8_t)(a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3));
                col[3] = (uint8_t)((xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3));
            }
        }
        add_round_key(s, ctx->rk + round * 16);
    }
    sar_memcpy(out, s, 16);
    return 1;
}

int aes_decrypt_block(void *vctx, const uint8_t *in, uint8_t *out) {
    aes_ctx_t *ctx = (aes_ctx_t *)vctx;
    uint8_t s[16];
    sar_memcpy(s, in, 16);
    add_round_key(s, ctx->rk + ctx->nr * 16);
    for (int round = ctx->nr - 1; round >= 0; round--) {
        uint8_t t;
        t = s[13]; s[13] = s[9]; s[9] = s[5]; s[5] = s[1]; s[1] = t;
        t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
        t = s[3]; s[3] = s[7]; s[7] = s[11]; s[11] = s[15]; s[15] = t;
        for (int i = 0; i < 16; i++) s[i] = INV_SBOX[s[i]];
        add_round_key(s, ctx->rk + round * 16);
        if (round != 0) {
            for (int c = 0; c < 4; c++) {
                uint8_t *col = s + c * 4;
                uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
                col[0] = (uint8_t)(gmul(a0,14) ^ gmul(a1,11) ^ gmul(a2,13) ^ gmul(a3,9));
                col[1] = (uint8_t)(gmul(a0,9) ^ gmul(a1,14) ^ gmul(a2,11) ^ gmul(a3,13));
                col[2] = (uint8_t)(gmul(a0,13) ^ gmul(a1,9) ^ gmul(a2,14) ^ gmul(a3,11));
                col[3] = (uint8_t)(gmul(a0,11) ^ gmul(a1,13) ^ gmul(a2,9) ^ gmul(a3,14));
            }
        }
    }
    sar_memcpy(out, s, 16);
    return 1;
}

int aes_master_from_window(uint32_t key_len, const uint8_t *window,
                           int base_block, uint8_t *master_out) {
    uint32_t nk = key_len / 4;
    if (nk != 4 && nk != 6 && nk != 8)
        return 0;
    if (base_block < 1)
        return 0;

    uint8_t cur[8][4];
    uint8_t prev[8][4];
    for (uint32_t i = 0; i < nk; i++)
        sar_memcpy(cur[i], window + i * 4, 4);

    for (int b = base_block; b >= 1; b--) {
        for (int off = (int)nk - 1; off >= 0; off--) {
            uint8_t wi[4], wim1[4], temp[4];
            sar_memcpy(wi, cur[off], 4);
            if (off >= 1)
                sar_memcpy(wim1, cur[off - 1], 4);
            else
                sar_memcpy(wim1, prev[nk - 1], 4);
            sar_memcpy(temp, wim1, 4);
            if (off == 0) {
                uint8_t r = temp[0];
                temp[0] = (uint8_t)(SBOX[temp[1]] ^ RCON[b]);
                temp[1] = SBOX[temp[2]];
                temp[2] = SBOX[temp[3]];
                temp[3] = SBOX[r];
            } else if (nk > 6 && off == 4) {
                temp[0] = SBOX[temp[0]]; temp[1] = SBOX[temp[1]];
                temp[2] = SBOX[temp[2]]; temp[3] = SBOX[temp[3]];
            }
            for (int j = 0; j < 4; j++)
                prev[off][j] = (uint8_t)(wi[j] ^ temp[j]);
        }
        for (uint32_t i = 0; i < nk; i++)
            sar_memcpy(cur[i], prev[i], 4);
    }

    for (uint32_t i = 0; i < nk; i++)
        sar_memcpy(master_out + i * 4, cur[i], 4);
    return 1;
}

void aes_word_byteswap(uint8_t *dst, const uint8_t *src, uint32_t len) {
    for (uint32_t i = 0; i + 4 <= len; i += 4) {
        dst[i + 0] = src[i + 3];
        dst[i + 1] = src[i + 2];
        dst[i + 2] = src[i + 1];
        dst[i + 3] = src[i + 0];
    }
}
